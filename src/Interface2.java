...
public class Interface2 {
	...
	JLabel clientResult;
	JLabel res,lblFunctionResultIn,resS,lblMajorityVoiting,major,status,sColor,sColor2,sColorS;
	
	static int popular;
	 int targetNumber = 0;
	static int func = 0;
	static int func2 = 0; //client 1 için
	static int funcS = 0; //server için
	private JLabel lblClent;
	private JLabel lblClient;
	private JLabel lblServer;
	private JLabel lblFunctionResultOf;
	private JLabel res2;
	...
	private void initialize() {
		 ...
		 Submit.addActionListener(new ActionListener()
			...
			public void actionPerformed(ActionEvent arg0) {
				// TODO Auto-generated method stub
				 ...
				 System.out.println("FUNCTION RESULT IN CLIENT2( f(x)=x*x )-->  " + func);
				 sentence = textField.getText();
					...
					try {
						...
						// if(func==Integer.parseInt(modifiedSentence))
		...
		sColor2.setBounds(92, 433, 73, 65);
		...
		lblServer.setBounds(400, 380, 158, 40);
		frame.getContentPane().add(lblServer);
		
		lblFunctionResultOf = new JLabel("Function Result of Client 1:");
		lblFunctionResultOf.setFont(new Font("Tahoma", Font.BOLD, 16));
		lblFunctionResultOf.setBounds(48, 150, 238, 37);
		frame.getContentPane().add(lblFunctionResultOf);
		
		res2 = new JLabel("");
		res2.setFont(new Font("Tahoma", Font.BOLD, 16));
		res2.setBounds(298, 150, 176, 37);
		...
		lblMachineIsOff.setBounds(217, 510, 173, 30);
	...
	public static boolean calc() {
		try {
			...
			if (func != popular) {
				System.out.println("There is an error at the client 2! ");
				return false;
			}
		} catch (IOException e) {
			e.printStackTrace();
